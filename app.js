const hexDigits = {
  ...
  let inputButtons = document.querySelectorAll(".input_button");
  let outputButtons = document.querySelectorAll(".output_button");
  
  for (let btn of inputButtons) {
    btn.addEventListener('click', (e) => {
  
      const active = document.querySelector(".active_in");
      
      if (active) {
        active.classList.remove("active_in");
      } 
      e.target.classList.add("active_in");
    });
  }
  
  for (let button of outputButtons) {
      button.addEventListener('click', (e) => {
    
        const active = document.querySelector(".active_out");
        
        if (active) {
          active.classList.remove("active_out");
        } 
        e.target.classList.add("active_out");
      });
    }
